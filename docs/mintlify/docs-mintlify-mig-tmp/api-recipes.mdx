...
`content_type=all` can return OCR text, audio transcripts, input events, accessibility data, app names, window titles, and browser URLs.
...
use frame endpoints when you already have a `frame_id` from search and need the OCR text, surrounding context, or metadata.
...
elements come from the accessibility tree. they are useful for finding buttons, links, fields, and UI labels that OCR may miss.
